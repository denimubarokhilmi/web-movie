<template>
  <div class="compMovies container-md p-1">
    <h6 class="text-uppercase streaming text-secondary mt-4">Movies</h6>
    <h2 class="section-title">
      <span class="highlight">All Movies</span>
    </h2>
    <div
      ref="newRelase"
...
const pathAPI = `https://api.themoviedb.org/3/movie/popular?language=en-US&page=${1}`;
(async function () {
  try {
...
async function BtnMoreMovie(ev) {
  const res = await index.callAPi(
    `https://api.themoviedb.org/3/movie/popular?language=en-US&page=${count.value}`
  );
